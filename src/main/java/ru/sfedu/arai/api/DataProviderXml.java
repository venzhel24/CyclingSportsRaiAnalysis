...
public class DataProviderXml implements DataProvider{
    ...
    public boolean addRaceRecord(Race race){
        try{
            ...
            List<Race> raceList = readFile(Race.class);
            raceList.add(race);
            writeFile(raceList, Race.class);
    ...
    public boolean deleteParticipantRecord(String id){
        try{
            ...
            List<Participant> participantsList = readFile(Participant.class);
            List<Participant> participantListCopy = List.copyOf(participantsList);
            participantsList.removeIf(participant -> participant.getId().equals(id));

            if(participantsList.equals(participantListCopy)){
                throw new Exception("Participant with id " + id + " not found");
            }

            writeFile(participantsList, Participant.class);
            log.debug("Participant deleted[2]: ".concat(participantListCopy
                    .stream()
    ...
    public boolean deleteRaceRecord(String id){
        try{
            ...
            List<Race> raceList = readFile(Race.class);
            List<Race> raceListCopy = List.copyOf(raceList);
            raceList.removeIf(race -> race.getRaceId().equals(id));

            if(raceList.equals(raceListCopy)){
                throw new Exception("Race with id " + id + " not found");
            }

            writeFile(raceList, Race.class);
            log.debug("Race deleted[2]: ".concat(raceListCopy
                    .stream()
    ...
    public boolean updateRaceRecord(Race race, String id){
        try{
            ...
            List<Race> raceList = readFile(Race.class);
            int index = raceList.indexOf(getRaceById(id).get());
            raceList.set(index, race);
            writeFile(raceList, Race.class);
            log.debug("Updating complete[2]: ");
            return true;
        }
    ...
    public Optional<Participant> getParticipantById(String id) {
        try {
            ...
            List<Participant> participantsList = readFile(Participant.class);
            Participant participantBean = participantsList.stream().filter(x -> (x).getId().equals(id)).findAny().get();
    ...
    public Optional<Race> getRaceById(String id) {
        try {
            log.debug("getRaceId[1]:");
            List<Race> raceList = readFile(Race.class);
            Race raceBean = raceList.stream().filter(x -> (x).getRaceId().equals(id)).findAny().get();
    ...
    private <T> void writeFile(List<T> beans, Class<?> clazz)  {
        try {
            ...
            File source = new File(CheckFile(clazz).toString());
            Writer writer = new FileWriter(source);
            if(beans.isEmpty()) {
                writer.write("");
                log.debug("Saving beans is empty");
                return;
            }
    ...
    private <T> List<T> readFile(Class<?> clazz){
        ...
        try{
            ...
            if(source.length() != 0) {
                ...
                log.debug("Empty beans loaded[2]: ");
            }
        }
        catch(Exception e){
